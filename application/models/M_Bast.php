...
class M_Bast extends CI_Model {
   ...
   function get_p8_bast($no_spk,$id){
        $q = $this->db->select("A.*,TO_CHAR(A.TGL_BAST, 'MM/DD/YYYY') TGL_BAST2, TO_CHAR(A.TGL_KL, 'MM/DD/YYYY') TGL_KL2, TO_CHAR(A.TGL_SPK, 'DD/MM/YYYY') TGL_SPK2, TO_CHAR(A.RECC_START_DATE, 'MM/DD/YYYY') RECC_START_DATE2, TO_CHAR(A.RECC_END_DATE, 'MM/DD/YYYY') RECC_END_DATE2 ")
                      ->from('PRIME_BAST_HGN A')
                      ->where('NO_SPK',$no_spk)
                      ->where('EXIST','1')
                      ->where('ID_BAST !=',$id)
